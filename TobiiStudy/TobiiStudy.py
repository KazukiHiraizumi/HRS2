...
sock1=WebsocketServer(port=5001, host="localhost")
client1=None
...
#5002 ET data server
sock2=WebsocketServer(port=5002, host="localhost")
client2=None
...
#    if d["left_gaze_point_validity"]>0 and d["right_gaze_point_validity"]>0:
    if d["left_gaze_point_validity"]>0 or d["right_gaze_point_validity"]>0:
      ...
      if d["right_gaze_point_validity"]>0: result["right"].append((d["right_gaze_point_on_display_area"][0]-0.5)*2)
      else: result["right"].append(0)      
  result["tm1"]=gaze_cmd["tm1"]
  result["tm2"]=gaze_cmd["tm2"]
  res=re.sub("'",'"',str(result))
  if client2 is not None:
    print("send gaze data")
    sock2.send_message(client2,res)
